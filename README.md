...
3. A **local MQTT broker** (Mosquitto) running on your server/PC receives the MQTT publish from A9G.
4. A **bridge script**  subscribes to relevant MQTT topic(s) on the local broker.
...
* Raw data from remote A9G modules will reach local broker.
